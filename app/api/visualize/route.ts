...
Generate a new photo of this exact same person based on the following request: "${description}"`
    ...
    // Explicit model validation - ensure we're using the correct endpoint
    console.log(`[${new Date().toISOString()}] Model config validation:`, {
      modelName,
      expectedPricing: '~$0.039/image',
    ...
    try {
      ...
      if (geminiError instanceof Error && geminiError.message?.includes('Unexpected token') && geminiError.message?.includes('<!DOCTYPE')) {
        throw new Error('AI service returned an HTML error page. This usually means: 1) Invalid API key, 2) Model not available, or 3) API temporarily unavailable.')
      }
    ...
    // Response model validation - log what Google actually used
    console.log(`[${new Date().toISOString()}] Response validation:`, {
      requestedModel: modelName,
      processingTime,
      responseMetadata: 'gemini-api-response'
    })
    ...
    if (images.length === 0) {
      ...
      return NextResponse.json(
        { 
          error: 'Image generation failed. The AI model did not return generated images. This could be due to content safety filters or prompt issues.',
          debug: {
    ...
    try {
      ...
      if (error instanceof Error && (error.message?.includes('404') || error.message?.includes('NOT_FOUND') || error.message?.includes('not found'))) {
        return NextResponse.json(
          { 
            error: 'Model not found or not accessible. The AI model may not be available in your region or project.',
            processingTime: errorTime,
            debug: {
